...
def ch2llr(ch, sigma2):
    """
    ch is the channel observation, sigma2 is channel noise variance
    function returns ln(P(x=0|ch)/ P(x=1|ch)) = ln(P(ch|x=0)/ P(ch|x=1))
    """
    return 2.0/sigma2*ch

@jit(nopython=True)
def bpsk(x):
    """x is a binary vector, 0 is mapped into 1 and 1 into -1"""
    return 1.0 - 2.0*x
...
def sim(standard, rate, z, ptype='A', N_MEASUREMENTS=24, C_AWGN_OFFSET=1.0, P_STEP=100.0,
    ...
    for datapoint in range(N_MEASUREMENTS): 
        ...
        while nblockerrors < MIN_ERRORS:
            ...
            x = mycode.encode(u)
            xm = bpsk(x) # modulate to BPSK

            (y,sigma2) = awgn(xm, SNR)

            yl = ch2llr(y, sigma2)
            (app,nit) = mycode.decode(yl, 'sumprod2')
            # app is ln(P(x=0|ch, checks all satisfied)/ P(x=1|ch, checks all satisfied))
            xh = (app < 0.0) # if app<0, bit-wise MAP estimate is BPSK symbol -1 which 
            # corresponds to binary bit 1; otherwise MAP estimate is BPSP symbol +1 
            # which corresponds to binary bit 0
...
if __name__ == "__main__":
    ...
    # A single star * unpacks a sequence or collection into positional arguments.
