...
C executables are in subdirectory `bin/` (if there isnt `bin/` create an empty one yourself)
...
Shirley: importantly, run these on the machine on which you want to run the decoder.
The current version works on linux machines (goneril) but I havent managed to get them
to work on Mac M1 chip.
